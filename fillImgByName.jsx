...
function main() {
  ...
  for (var i = 0; i < myText.length; i++) {
    ...
    titlesInfo[i].push(myText[i].contents);
    // alert(myText[i].parentTextFrames[0].parentPage);
    titlesInfo[i].push(myText[i].parentTextFrames[0].parentPage.id);
  }
...
function findTitle(myOne, titlesInfo) {
  for (var i = 0; i < titlesInfo.length; i++) {
    if (myOne.parentPage.id >= titlesInfo[i][1] && myOne.parentPage.id < titlesInfo[i + 1][1]) {

      return titlesInfo[i][0];
      break;
